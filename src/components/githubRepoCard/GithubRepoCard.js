...
export default function GithubRepoCard({ repo }) {
  ...
  let description = repo.node.description;
  let urlRegex = /(https?:\/\/[^ ]*)/;
  let url = description.match(urlRegex)[1];
  let updated_descripttion = description.replace(url, "");
    ...
    <Fade bottom duration={1000} distance="20px">
      <div>
        ...
        >
          ...
          <p className="repo-description">{updated_descripttion}</p>
          <p className="demolink">
            <a
              style={{ fontWeight: "bold" }}
              onClick={(event) => openDemoApp(event, url)}
            >
              Live Demo
            </a>
          </p>
